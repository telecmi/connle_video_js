import { Room, VideoPresets, setLogLevel, LogLevel, AudioSession, AndroidAudioTypePresets, getDefaultAppleAudioConfigurationForMode } from './livekit';
const RTC_CONFIG = {
  iceServers: [{
...
export default class Video {
  ...
  async configureAudioSession(audio, video, _this) {
    ...
    try {
      if (typeof AudioSession.configureAudio === 'function') {
        const audioConfig = {
          ios: {
            defaultOutput: preferSpeaker ? 'speaker' : 'earpiece'
          }
        };
  ...
  async connect(audio = true, video = false, token, _this) {
    ...
    this.room.on('mediaDevicesError', error => {
      this.emitError(_this, {
        ...
        message: error?.message || 'media_device_error'
      });
    });
    ...
    this.room.on('mediaConnectionError', error => {
      this.emitError(_this, {
        ...
        message: error?.message || 'media_connection_error'
      });
    });
    ...
    this.room.on('trackPublicationFailed', error => {
      this.emitError(_this, {
        ...
        message: error?.message || 'track_publication_failed'
      });
    });
    ...
    try {
      ...
      this.emitError(_this, {
        code: 'MEDIA_CONNECTION_ERROR',
        message: error?.message || 'connection_failed'
      });
      return;
    }
    ...
    if (audio) {
      try {
        ...
        this.emitError(_this, {
          code: 'MEDIA_CONNECTION_ERROR',
          message: error?.message || 'microphone_publish_failed'
        });
        return;
      }
    }
    if (video) {
      try {
        ...
        this.emitError(_this, {
          code: 'MEDIA_CONNECTION_ERROR',
          message: error?.message || 'camera_publish_failed'
        });
        return;
      }
    }
  ...
  async mute(_this) {
    ...
    if (this.localParticipant.isMicrophoneEnabled) {
      try {
        ...
        this.emitError(_this, {
          code: 'MUTE_ERROR',
          message: error?.message || 'mute_failed'
        });
      }
    } else {
      this.emitError(_this, {
  ...
  async unmute(_this) {
    ...
    if (!this.localParticipant.isMicrophoneEnabled) {
      try {
        ...
        this.emitError(_this, {
          code: 'UNMUTE_ERROR',
          message: error?.message || 'unmute_failed'
        });
      }
    } else {
      this.emitError(_this, {
  ...
  async pause(_this) {
    ...
    if (this.localParticipant.isCameraEnabled) {
      try {
        ...
        this.emitError(_this, {
          code: 'VIDEO_PAUSE_ERROR',
          message: error?.message || 'pause_failed'
        });
      }
    } else {
      this.emitError(_this, {
    ...
    if (!this.localParticipant.isScreenShareEnabled) {
      try {
        ...
        this.emitError(_this, {
          ...
          message: error?.message || 'screen_share_failed'
        });
      }
    } else {
      this.emitError(_this, {
  ...
  async stopScreenShare(_this) {
    ...
    if (this.localParticipant.isScreenShareEnabled) {
      try {
        ...
        this.emitError(_this, {
          code: 'SCREEN_UNSHARE_ERROR',
          message: error?.message || 'screen_unshare_failed'
        });
      }
    } else {
      this.emitError(_this, {
  ...
  async setSpeaker(enabled, _this) {
    ...
    try {
      if (typeof AudioSession.selectAudioOutput === 'function') {
        // Output device ids differ by platform (iOS: default/force_speaker,
        // Android: speaker/earpiece). Pick a valid one from what's available.
        ...
        if (typeof AudioSession.getAudioOutputs === 'function') {
          ...
          if (Array.isArray(outputs) && outputs.length) {
            if (enabled) {
              target = outputs.includes('force_speaker') ? 'force_speaker' : outputs.includes('speaker') ? 'speaker' : target;
            } else {
              target = outputs.includes('earpiece') ? 'earpiece' : outputs.includes('default') ? 'default' : target;
            }
          }
        }
        await AudioSession.selectAudioOutput(target);
      }
      if (_this) _this.emit('speakerChanged', {
        speaker: this.speakerOn
      });
    } catch (error) {
      this.emitError(_this, {
        code: 'SPEAKER_ERROR',
        message: error?.message || 'speaker_failed'
      });
    }
  }
  toggleSpeaker(_this) {
  ...
  async switchCamera(_this) {
    ...
    let track = null;
    try {
      pub = typeof this.localParticipant.getTrackPublication === 'function' ? this.localParticipant.getTrackPublication('camera') : null;
    ...
    if (!track) {
      this.emitError(_this, {
        code: 'NO_CAMERA',
        message: 'No active camera to switch'
      });
      return;
    }
    ...
    const finish = () => {
      ...
      try {
        if (pub) this.emitLocalTrack(_this, pub);
      } catch (_e) {}
      if (_this) _this.emit('cameraSwitched', {
