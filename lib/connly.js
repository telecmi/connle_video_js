...
export default class ConnlySignalling extends EventEmitter {
  ...
  deliverIncomingCall(data) {
    ...
    try {
      ...
      this.emitError({
        ...
        message: error?.message || 'incoming_handler_failed'
      });
      return false;
    }
  }
  ...
  connectMedia(audio, video, token) {
    if (!token) {
      this.emitError({
        code: 'MEDIA_TOKEN_MISSING',
        message: 'Media token missing'
      });
      return;
    }
    this.video.connect(audio, video, token, this).catch(error => {
      this.emitError({
        ...
        message: error?.message || 'media_connect_failed'
      });
    });
  }
  answer(callback) {
    ...
    if (!this.callId) {
      this.emitError({
        code: 'NO_CALL_INVITE',
        message: 'No call invite'
      });
      return;
    }
    if (this.answerInFlight) return;
    this.answerInFlight = true;
    this.socket.emit('connle_answer_call', {
      call_id: this.callId
    }, ack => {
      if (ack?.code === 200) {
  ...
  reject(callback) {
    ...
    this.socket.emit('connle_reject_call', {
      call_id: this.callId
    }, ack => {
      this.rejectInFlight = false;
      if (callback) callback(ack);
  ...
  cancelOutgoingCall(callback_function) {
    if (!this.isConnected) {
      this.emitError({
        code: 'NOT_CONNECTED',
        message: 'Not connected to signalling'
      });
      return;
    }
    if (this.callId) {
